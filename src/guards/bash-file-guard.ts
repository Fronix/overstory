...
const capArg = capIdx !== -1 ? process.argv[capIdx + 1] : undefined;
if (capArg) {
	capability = capArg;
}

const safePrefixIdx = process.argv.indexOf("--safe-prefixes");
const safePrefixArg = safePrefixIdx !== -1 ? process.argv[safePrefixIdx + 1] : undefined;
if (safePrefixArg) {
	extraSafePrefixes = safePrefixArg.split(",").filter(Boolean);
}
